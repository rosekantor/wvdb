...
def batch_extract_hosts_lineages(genbank_ids, batch_size=100):
    ...
    for i in range(0, len(genbank_ids), batch_size):
        ...
        print(f"  Entrez query batch {i // batch_size + 1} "
...
def get_host_lineage(corent_df, cache_path, target_col='tname_nt'):
    ...
    Reads from cache if it exists; writes cache after querying.
    """
    ...
    if hit_level_col not in corent_df.columns:
        ...
        if hit_level_col is None:
            ...
            return pd.DataFrame(columns=[
                'contig', target_col, 'hosts_ntBlastHit',
    ...
    genbank_ids = list(for_entrez[target_col].unique())
    print(f"  Querying Entrez for {len(genbank_ids)} unique GenBank IDs...",
          file=sys.stderr)
    raw = batch_extract_hosts_lineages(genbank_ids)
    ...
    results_df['lowest_tax_ntBlastHit'] = (
        results_df['lineage_ntBlastHit'].str.extract(r'.*;(.*$)')
    )
    results_df = for_entrez[['contig', target_col]].merge(
        results_df, on=target_col, how='left'
    )
...
def report(genomad_df, checkv_df, rdrpcatch_df, rnavirhost_df):
    ...
    provirus = genomad_df[genomad_df['contig'].str.contains('provirus', na=False)]
    high_kmer = checkv_df[checkv_df['kmer_freq'] > 1.2]

    if len(provirus) > 0:
        print(
            f"Warning: geNomad reports {len(provirus)} proviruses — "
            f"consider trimming: {provirus['contig'].tolist()}",
            file=sys.stderr
        )
    if len(high_kmer) > 0:
        print(
