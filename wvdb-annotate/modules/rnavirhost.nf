 ...
 * then runs `rnavirhost predict`. The input FASTA is the canonical source
 * of contig IDs for the orders file — CheckV output is not used here
 * since its only role was providing a contig list, and the FASTA itself
 * is a more reliable source (avoids row-count mismatches vs RNAVirHost's
 * strict 1:1 requirement between FASTA and orders.csv).
 *
